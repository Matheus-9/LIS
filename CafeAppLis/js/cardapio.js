//Importa o caminho que se encontra dentro do diretório model os arquivos .js defenido: (dataset.js e o foods.js).
import itens from './model/dataset.js';
import foodsModel from './model/foods.js';

foodsModel.load(itens); // Acessa os itens por meio de um load que se encontra dentro do arquivo (foods.js)
...
function initFoodsCard () {
  ...
  // Atribuição da variável para criação de um novo item do cardapio, gerado pelo (HTML) de forma dinâmica pelo bootstrap. Preenchendo a imagem id e description.
    ...
    /**
     * Analisa o texto especifico do texto como html e xml e insere os no que resultam na árvore DOM em uma posição especifica.
     *  Nesse caso insere um nó dentro do elemento, depois de seu último filho que no caso é um novo elemento view.
     */
...
foodForm.onsubmit = function (event) {
  ...
  event.preventDefault(); // O método preventDefault() cancela o evento se for cancelado, o que significa que a ação padrão que pertence ao evento não ocorrerá.

  let newFood = Object.fromEntries(new FormData(foodForm));
  foodsModel.create(newFood); // Quando já estão tratados com valor-chave, é possível uma criação do módulo foods. Possibilitando em criar um novo item que vai íncluido dentro da array. 

/**
   * A variável foi atribuida com o objetivo de transformar uma lista em pares de valor-chave em um objeto utilizando o Object.fromEntries.
   * que são representadas pelos campos de um elemento. Os dados são enviados entre valor-chave que possibilita uma fácil manipulação.
...
//Inicia o formulário com a função initFoodsCard()
